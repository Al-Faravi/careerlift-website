...
import { 
  ...
  CheckCircle2, MapPin, Phone, Mail, Quote, PlayCircle, ShieldCheck, ArrowRight, Clock, Sparkles, Tag, ChevronDown
} from 'lucide-react';
...
export default function Home() {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ name: '', phone: '', interest: '🎉 NSDA Free Course (Scholarship / স্কলারশিপ)', source: 'Home Page' });
  ...
  // ✅ 1. SMART AUTO-FLAG DETECTOR (Works automatically when Admin selects Country)
  ...
  const featuredCourses = [
    {
      id: "10", category: 'Healthcare & Beauty', image: '/Day long Primary Healthcare.png', duration: '1 Day (10 AM - 5 PM)',
      rating: '5.0', title: { EN: 'Day-long Primary Healthcare, First Aid & CPR', BN: 'দিনব্যাপী প্রাইমারি হেলথ কেয়ার, ফার্স্ট এইড ও সিপিআর' },
      tag: { EN: 'Certificate Included', BN: 'সার্টিফিকেট প্রদান' }
    },
    {
      id: "11", category: 'Healthcare & Beauty', image: '/CPR & Emmergency Response Program.jpg', duration: '1 Day (10 AM - 5 PM)',
      rating: '5.0', title: { EN: 'CPR & Emergency Response Training Program', BN: 'CPR ও ইমার্জেন্সি রেসপন্স ট্রেনিং প্রোগ্রাম' },
      tag: { EN: 'Hands-on Training', BN: 'হ্যান্ডস-অন ট্রেনিং' }
    },
    {
      id: "5", category: 'Healthcare & Beauty', image: '/Primary healthcare.png', duration: '3 Months',
      rating: '4.9', title: { EN: 'Primary Healthcare Service Level 2', BN: 'প্রাইমারি হেলথকেয়ার সার্ভিস লেভেল-২' },
      tag: { EN: 'NSDA Certified', BN: 'NSDA অনুমোদিত' }
    }
  ];

  // ✅ FUTURE CRUD READY: PACKAGES WITH AUTO-FLAG LOGIC
  const featuredPackages = [
    ...
    },
    {
      ...
      badge: "Free Tuition in Germany", price: { EN: "Low Bank Show & High Ratio", BN: "সহজ স্পন্সর ও সর্বোচ্চ ভিসা হার" },
  ...
  // ✅ SUCCESS STORIES FOR INFINITE COMMENT MARQUEE
  ...
  return (
    <div className="pb-16 bg-slate-50 min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16 sm:space-y-24 pt-6">
        ...
        <section>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
            {servicesList.map((srv, idx) => {
              const icons = [
                <Target size={22} className="text-white sm:w-7 sm:h-7" strokeWidth={2} />,
                <GraduationCap size={22} className="text-white sm:w-7 sm:h-7" strokeWidth={2} />,
                <PlaneTakeoff size={22} className="text-white sm:w-7 sm:h-7" strokeWidth={2} />,
                <Stethoscope size={22} className="text-white sm:w-7 sm:h-7" strokeWidth={2} />
              ];
              const gradients = ["from-blue-600 to-indigo-600", "from-emerald-600 to-teal-600", "from-purple-600 to-indigo-600", "from-amber-600 to-orange-600"];
              
              return (
                <NeoCard key={idx} className="flex flex-col justify-between h-full bg-white hover:bg-slate-50/80 group border-slate-200/80 shadow-sm hover:shadow-xl transition-all duration-300 rounded-2xl sm:rounded-3xl p-4 sm:p-7">
                  <div>
                    <div className={`w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-gradient-to-tr ${gradients[idx % gradients.length]} flex items-center justify-center shadow-md mb-3 sm:mb-6 group-hover:scale-110 transition-transform duration-300`}>
                      {icons[idx % icons.length]}
                    </div>
                    <h3 className="text-sm sm:text-xl font-black text-slate-800 mb-1 sm:mb-3 group-hover:text-blue-600 transition-colors leading-tight line-clamp-2">{srv.title}</h3>
                    <p className="text-[11px] sm:text-sm text-slate-600 leading-relaxed font-medium line-clamp-3 sm:line-clamp-none">{srv.desc}</p>
                  </div>
                  
                  <div className="mt-4 sm:mt-8 pt-3 sm:pt-5 border-t border-slate-100">
                    <Link to={srv.link || "/courses"} className="flex items-center text-[11px] sm:text-sm font-black text-slate-900 hover:text-blue-600 transition-colors group/link">
        ...
        <section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-8">
            {featuredCourses.map((course, idx) => (
              <NeoCard key={idx} className="!p-0 flex flex-col bg-white hover:bg-slate-50/50 border-slate-200/80 shadow-sm hover:shadow-xl group relative overflow-hidden transition-all duration-300 rounded-2xl sm:rounded-3xl">
                <div className="relative h-32 sm:h-56 w-full overflow-hidden bg-slate-100 border-b border-slate-100">
                  <img src={course.image} alt={course.title[lang] || course.title.EN} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" onError={(e) => { e.target.src = 'https://images.unsplash.com/photo-1522202176988-66273c2fd55f?auto=format&fit=crop&w=600&q=80' }} />
                ...
                <div className="p-3.5 sm:p-8 flex flex-col flex-grow">
                  <h3 className="text-xs sm:text-xl font-black text-slate-900 mb-2 sm:mb-4 group-hover:text-blue-600 transition-colors line-clamp-2 leading-snug">
                    {course.title[lang] || course.title.EN}
                  </h3>
                  <div className="flex flex-wrap gap-1 sm:gap-2 mb-3 sm:mb-6">
                    <span className={`flex items-center space-x-1 px-2 sm:px-3 py-0.5 sm:py-1 rounded-lg sm:rounded-xl text-[10px] sm:text-xs font-bold border ${
                      course.tag.EN.includes('Certificate') || course.tag.EN.includes('Hands') || course.tag.EN.includes('NSDA') ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-50 text-slate-600 border-slate-200'
                    }`}>
                      <CheckCircle2 size={12} className="text-emerald-600 flex-shrink-0" />
                      <span className="truncate">{course.tag[lang] || course.tag.EN}</span>
                    </span>
                  </div>
                  <div className="mt-auto pt-3 sm:pt-5 border-t border-slate-100 flex items-center justify-between">
                    ...
                    <Link to={`/course/${course.id}`}>
                      <button className="px-3 sm:px-6 py-1.5 sm:py-3 rounded-lg sm:rounded-xl bg-slate-900 text-white font-black text-[10px] sm:text-xs shadow-md hover:bg-blue-600 active:scale-95 transition-all flex items-center space-x-1 group/btn">
                        <span>{lang === 'EN' ? 'Details' : 'বিস্তারিত'}</span>
        ...
        {/* 4.2 FEATURED STUDY ABROAD & VISA PACKAGES */}
        <section>
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 sm:mb-12 gap-3">
            <SectionTitle 
              subtitle={lang === 'EN' ? "Global Opportunities" : "আন্তর্জাতিক প্যাকেজ ও অফার"} 
              title={lang === 'EN' ? "Featured Study Abroad & Visa Packages" : "আমাদের স্পেশাল স্টাডি ও ভিসা প্যাকেজসমূহ"} 
              description={lang === 'EN' ? "Explore our exclusive credit transfer pathways, free tuition options, and work-permit assured global packages." : "স্বল্প খরচে ব্রিটিশ ডিগ্রি, ইউরোপে ফ্রি পড়াশোনা এবং নিশ্চিত জবের সুযোগসহ আমাদের বিশেষ প্যাকেজসমূহ।"} 
              center={false} 
            />
            <Link to="/study-abroad#packages" className="self-start md:self-end">
              <NeoButton variant="secondary" className="!py-2.5 !px-5 sm:!py-3.5 sm:!px-7 text-xs sm:text-sm font-bold whitespace-nowrap shadow-md !bg-slate-900 !text-white hover:!bg-blue-600">
                {lang === 'EN' ? 'Explore All Packages →' : 'সকল প্যাকেজ দেখুন →'}
              </NeoButton>
            </Link>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-8">
            {featuredPackages.map((pkg, idx) => {
              ...
              return (
                <NeoCard key={idx} className="flex flex-col justify-between h-full bg-white hover:bg-blue-50/20 border-slate-200/80 shadow-sm hover:shadow-xl group transition-all duration-300 rounded-2xl sm:rounded-3xl p-4 sm:p-8 relative overflow-hidden">
                  <div>
                    <div className="flex flex-wrap items-center justify-between gap-1.5 mb-3 sm:mb-4">
                      <span className="text-[10px] sm:text-xs font-black text-slate-800 bg-slate-100 border border-slate-200/80 px-2 sm:px-3 py-1 rounded-full flex items-center space-x-1.5">
                        <img src={flagSrc} alt="flag" className="w-4 h-3 object-cover rounded shadow-2xs" />
                        <span>{pkg.country}</span>
                      </span>
                      <span className="text-[9px] sm:text-[10px] font-extrabold uppercase bg-amber-500/10 text-amber-600 border border-amber-500/20 px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full flex items-center">
                        ★ {pkg.badge}
                      </span>
                    </div>

                    <h3 className="text-xs sm:text-xl font-black text-slate-900 mb-1 sm:mb-2 group-hover:text-blue-600 transition-colors leading-snug line-clamp-2">
                      {pkg.title[lang]}
                    </h3>
                    
                    <span className="text-[10px] sm:text-xs font-bold text-emerald-600 block mb-2 sm:mb-3 truncate">
                      ✔ {pkg.price[lang]}
                    </span>

                    <p className="text-[11px] sm:text-sm text-slate-600 leading-relaxed font-medium line-clamp-3 sm:line-clamp-none">
                      {pkg.desc[lang]}
                    </p>
                  </div>

                  <div className="mt-4 sm:mt-8 pt-3 sm:pt-5 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2">
                    <span className="text-[10px] sm:text-xs font-bold text-slate-400 flex items-center">
                      <Clock size={11} className="mr-1 text-blue-500" /> {pkg.intake}
                    </span>
                    <Link to={pkg.link} className="flex items-center text-[11px] sm:text-sm font-black text-blue-600 hover:text-blue-800 transition-colors group/link">
                      <span>{lang === 'EN' ? 'Apply Now' : 'আবেদন করুন'}</span>
        ...
        {/* 4.5 LIFE AT CAREERLIFT GALLERY */}
        <section className="bg-slate-900 rounded-[2rem] sm:rounded-[2.5rem] p-5 sm:p-14 text-white overflow-hidden relative shadow-xl">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 relative z-10 gap-3">
            ...
            <Link to="/about-us">
              <button className="flex items-center space-x-2 text-xs sm:text-sm font-bold text-blue-400 hover:text-white bg-white/10 hover:bg-white/20 px-4 py-2.5 rounded-xl border border-white/15 transition-all">
                <PlayCircle size={16} />
                <span>{lang === 'EN' ? 'Explore Infrastructure →' : 'ক্যাম্পাস দেখুন →'}</span>
              </button>
            </Link>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 relative z-10">
            <div className="col-span-2 row-span-2 rounded-2xl sm:rounded-3xl overflow-hidden group relative aspect-square sm:aspect-auto border border-white/10">
              ...
              <div className="absolute bottom-4 left-4 text-white">
                <span className="text-[9px] sm:text-[10px] font-bold uppercase bg-blue-600 px-2 py-0.5 rounded">Practical Facility</span>
                <h4 className="font-black text-sm sm:text-lg mt-1">Modern Computer & IT Lab</h4>
              </div>
            </div>
            <div className="rounded-2xl sm:rounded-3xl overflow-hidden group relative h-36 sm:h-52 border border-white/10">
              ...
              <div className="absolute bottom-2 left-2 sm:bottom-3 sm:left-3 text-white text-[10px] sm:text-xs font-bold">Collaborative Study</div>
            </div>
            <div className="rounded-2xl sm:rounded-3xl overflow-hidden group relative h-36 sm:h-52 border border-white/10">
              ...
              <div className="absolute inset-0 bg-slate-950/40 group-hover:bg-transparent transition-colors"></div>
              <div className="absolute bottom-2 left-2 sm:bottom-3 sm:left-3 text-white text-[10px] sm:text-xs font-bold">1-on-1 Counseling</div>
            </div>
            <div className="col-span-2 rounded-2xl sm:rounded-3xl overflow-hidden group relative h-36 sm:h-52 border border-white/10">
              ...
              <div className="absolute bottom-3 left-3 sm:bottom-4 sm:left-4 text-white">
                <span className="text-[9px] sm:text-[10px] font-bold uppercase bg-emerald-600 px-2 py-0.5 rounded">Seminars</span>
                <h4 className="font-bold text-xs sm:text-base mt-0.5 sm:mt-1">Visa & Scholarship Workshops</h4>
              </div>
            </div>
          </div>
        </section>

        {/* 5. SUCCESS STORIES: SMALL COMMENT MARQUEE */}
        <section className="overflow-hidden">
          <div className="mb-8 sm:mb-12 text-center max-w-2xl mx-auto">
            <span className="text-xs font-black uppercase tracking-widest text-emerald-600 bg-emerald-50 px-3.5 py-1 rounded-full border border-emerald-200 inline-block mb-2">
              ★ {lang === 'EN' ? 'Verified Reviews' : 'শিক্ষার্থীদের বাস্তব অভিজ্ঞতা'}
            </span>
            <h2 className="text-2xl sm:text-4xl font-black text-slate-900 tracking-tight">
              {t.storiesTitle || (lang === 'EN' ? "Success Stories & Feedback" : "সাফল্যের গল্প ও মতামত")}
            </h2>
            <p className="text-xs sm:text-sm text-slate-600 font-medium mt-1">
              {lang === 'EN' ? "Real reviews from students who transformed their careers and achieved global mobility with us." : "ক্যারিয়ারলিফটের হাত ধরে নিজেদের জীবন ও ক্যারিয়ার বদলে ফেলার বাস্তব অভিজ্ঞতা।"}
            </p>
          </div>
          ...
          <div className="relative w-full py-4 bg-slate-100/60 rounded-3xl border border-slate-200/60 overflow-hidden">
            ...
            <div className="animate-comment-marquee gap-4 flex items-center">
              {[...storiesList, ...storiesList, ...storiesList].map((story, idx) => (
                <div 
                  key={idx} 
                  className="w-72 sm:w-80 flex-shrink-0 p-4 sm:p-5 bg-white rounded-2xl border border-slate-200/80 shadow-2xs hover:shadow-md transition-all flex flex-col justify-between space-y-3"
                >
                  ...
                  <div className="flex items-center space-x-3 pt-3 border-t border-slate-100">
                    <div className="w-8 h-8 rounded-xl bg-slate-900 flex items-center justify-center text-white font-black text-xs shadow-2xs flex-shrink-0">
                      {story.initials || "CL"}
                    </div>
        ...
        <section id="consultation-form" className="bg-white border border-slate-200/80 shadow-xl rounded-[2rem] sm:rounded-[2.5rem] p-5 sm:p-14 overflow-hidden relative">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-14 items-start">
            ...
            <div className="lg:col-span-5 space-y-6 sm:space-y-8">
              ...
              <div className="space-y-3">
                <div className="flex items-start space-x-3 sm:space-x-4 p-3.5 sm:p-4 rounded-2xl bg-slate-50 border border-slate-200/70">
                  ...
                  <div>
                    <h4 className="text-[10px] sm:text-[11px] font-black uppercase tracking-widest text-slate-400">{t.addressTitle || (lang === 'EN' ? "Office Address" : "ঠিকানা")}</h4>
                ...
                <div className="flex items-start space-x-3 sm:space-x-4 p-3.5 sm:p-4 rounded-2xl bg-slate-50 border border-slate-200/70">
                  ...
                  <div>
                    <h4 className="text-[10px] sm:text-[11px] font-black uppercase tracking-widest text-slate-400">{t.hotlineTitle || (lang === 'EN' ? "Direct Hotline" : "হটলাইন")}</h4>
            ...
            <div className="lg:col-span-7 space-y-6">
              <div className="bg-slate-50 border border-slate-200 rounded-2xl sm:rounded-3xl p-5 sm:p-10 shadow-sm">
                <div className="mb-6">
                  <h3 className="text-lg sm:text-2xl font-black text-slate-800">{t.formTitle || (lang === 'EN' ? "Book Free Consultation" : "ফ্রি পরামর্শের জন্য রেজিস্ট্রেশন করুন")}</h3>
                ...
                {submitSuccess ? (
                  ...
                  <form onSubmit={handleFormSubmit} className="space-y-3.5 sm:space-y-4">
                    ...
                    {/* 👇 RESPONSIVE & MOBILE-OPTIMIZED CUSTOM DROPDOWN */}
                    <div className="relative w-full">
                      <select 
                        name="interest" 
                        value={formData.interest} 
                        onChange={handleInputChange} 
                        className="w-full pl-3.5 pr-10 py-3 sm:py-3.5 rounded-xl bg-white border border-slate-200 text-xs sm:text-sm font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer appearance-none truncate block shadow-2xs"
                      >
